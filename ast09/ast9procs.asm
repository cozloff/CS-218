...
push r13   
    ...
    ;lea r12, byte [rbp -(BUFFSIZE+5)] ;stack dynamic variable
    ;lea r13, byte [rbp - BUFFSIZE] ;beginning of 50 byte buffer
    ;mov word [r12], 1  ;mov number into stack variable EXAMPLE
    ;mov byte [r13], 2 ; mov number into beginning of buffer EXAMPLE
    ;movsx r10, word [r12] ;mov stack variable into register EXAMPLE 
    ;movsx r11, byte [r13] ;mov beginning of buffer into register EXAMPLE

    readCharacters:
        mov rax, SYS_read 
        mov rdi, STDIN
        movsx rsi, byte [rbp -(BUFFSIZE+5)]
        mov rdx, 1
        syscall
    ...
    mov qword [rdi], r10    ;Example for moving into number variable 
    mov rax, r11            ;Example for moving into ERROR CODE return 

    ;third, use the sys_read to test interacting with the OS
        ;mov rax, sys_read
        ;mov rdi, STDIN
        ;mov rsi, addCharbuff
        ;mov rdx, 1 ;one character at a time
        ;when you get the LF put the NULL in, if your buffer is full don't store them  
    ;convert 
